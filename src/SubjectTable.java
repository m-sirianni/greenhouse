public class SubjectTable {
	...
	public  SubjectTable() {
		root = new Node<Subject>(new Subject("root"));
	}
	...
	public synchronized void subscribe (String url, MessageQueue mq){
		...
		for (int i = 1; i < nodes.length; i++) {
			...
			if (curLvl.getChildren().isEmpty()) {
				...
				//System.out.println("Ho creato :" + child.getData().name);
	...
	private void unPrint(Node<Subject> node, String threadid, int lvl) {
		if (node.getChildren().isEmpty())
			return;
		
		for (Node<Subject> sub : node.getChildren()) {
			//System.out.println("Lvl: " + lvl + " " + sub.getData().name);
			unPrint(sub, threadid, lvl+1);
		}
		
	}
	
	
	public synchronized void printTable (String threadid){
		
		for(Node<Subject> node : root.getChildren()) {
			//System.out.println("Lvl: 1 " + node.getData().name);
			unPrint(node, threadid, 2);
		}
	...
	private void visita_rec(Node<Subject> node) {
		...
		for (Node<Subject> sub : node.getChildren()) {
			for(MessageQueue mq : sub.getData().subscriber){
				System.out.println(mq.getThId());
			}
			visita_rec(sub);
		}
